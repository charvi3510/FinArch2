...
def health_check():
    return {
        "status": "online",
        "service": "FINARCH AI Autonomous Financial Decision Engine",
        "version": "1.0.0"
    }
...
def get_health_score(db: Session = Depends(get_db)):
    profile = ProfileService.get_profile(db)
    return FinancialHealthEngine.calculate_health_score(profile)
...
def analyze_portfolio(db: Session = Depends(get_db)):
    profile = ProfileService.get_profile(db)
    return OpportunityOptimizer.analyze_portfolio_risk(profile)

@router.get("/risk", response_model=RiskAssessment)
def get_risk_assessment(db: Session = Depends(get_db)):
    profile = ProfileService.get_profile(db)
    return RiskEngine.assess_risk(profile)
...
def create_goal(goal_in: GoalBase, db: Session = Depends(get_db)):
    ...
    return GoalBase(
        ...
        target_year=new_goal.target_year,
        monthly_contribution=new_goal.monthly_contribution,
        priority=new_goal.priority,
        is_essential=new_goal.is_essential,
        completion_percentage=round(completion, 1),
        required_monthly=round(req_monthly, 2),
        is_on_track=new_goal.monthly_contribution >= req_monthly
    )
...
async def chat_with_advisor(req: ChatRequest, db: Session = Depends(get_db)):
    profile = ProfileService.get_profile(db)
    return await AIReasoningEngine.generate_response(profile, req)
